...
class TestTableViewController: UITableViewController {
    ...
    override func viewDidLoad() {
        ...
        //注册cell的xib
        tableView.registerNib(UINib(nibName: "CheckInTableViewCell", bundle: nil ), forCellReuseIdentifier: "checkInCell")
//        tableView.registerClass(CheckInTableViewCell.self, forCellReuseIdentifier: "checkIncell")
...
//    }
    ...
    func  likeCheckIntemp(sender:AnyObject) {
                ...
                var cellRow = getCellRow(sender,tableView: self.tableView)
                let row = cellRow[0] as! Int
                let selectCell = cellRow[1] as! CheckInTableViewCell
                let likeButton = selectCell.likeButton as UIButton
...
//        //        avatar.image = UIImage(named: <#T##String#>)
        cell.username.text = checkIn.valueForKey("uid")!.valueForKey("username") as? String
        cell.time.text = timeStringForMessage(checkIn.valueForKey("createdAt") as! NSDate)
        cell.likeButton.addTarget(self, action: "likeCheckIntemp:", forControlEvents: UIControlEvents.TouchUpInside)
        cell.content.text = checkIn.valueForKey("content") as? String
        cell.content.sizeToFit()
        
        cell.imageCheckInHeight.constant = 200
...
//        //checkIn插图
        ...
        if checkIn.valueForKey("image") != nil {
            ...
            cell.contentImageView.af_setImageWithURL(url!, placeholderImage: placeholderImage)
            
            //处理图片加载错乱问题
            let updateCell = tableView.cellForRowAtIndexPath(indexPath)
            if ((updateCell) != nil) {
                updateCell?.imageView?.image = cell.contentImageView.image
            }
